...
// CORS: разрешаем обращаться к API только со своего домена (защита от использования формы
// с чужих сайтов). Запросы с самого сайта — same-origin, для них CORS не применяется,
// поэтому форма работает всегда, даже на preview-деплоях и будущем своём домене.
const ALLOWED_ORIGINS = [
  'https://rimma.vercel.app',
  // ← добавьте сюда свой домен, когда подключите, напр. 'https://example.ru'
];
...
module.exports = async (req, res) => {
  // CORS на каждый ответ (заголовки держатся на res до отправки).
