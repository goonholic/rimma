...
module.exports = async (req, res) => {
  try {
    ...
    const code = p.Shp_service || p.shp_service || '';
    ...
    const shp = code ? ('Shp_service=' + code) : '';
    const expect = md5((shp ? [outSum, invId, PASS2, shp] : [outSum, invId, PASS2]).join(':'));
    if (expect !== sigIn) { res.statusCode = 400; return res.end('bad_sign'); }

    // Подпись верна → оплата настоящая. Шлём отстук (не мешает ответу "OK").
    ...
    if (TOKEN && CHAT) {
      ...
      const text =
        '✅ <b>Оплата получена</b>\n' +
        '━━━━━━━━━━━━━━\n' +
        '✨ <b>Услуга:</b> ' + esc(title) + '\n' +
        '💰 <b>Сумма:</b> ' + esc(outSum) + ' ₽\n' +
        '🧾 <b>Заказ №</b> ' + esc(invId);
      try {
